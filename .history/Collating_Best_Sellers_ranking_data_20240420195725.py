...
def parse_shelf_string(shelf_string):
    ...
    if match:
        ...
        shelf_date = datetime.datetime.strptime(shelf_date_str, '%Y-%m-%d').date()
...
def parse_string_to_dict(data_string):
    ...
    # 定义字符串中关键字段名
    keys = ['ASIN', '品牌', '卖家', '配送', '卖家', '加入产品库', '近30天销量(父体)', '近30天销量(子体)', '销售额', 'FBA费用', '毛利率', '变体数', '价格', 'Coupon', '评分(评分数)', 'Color', 'Pattern Name', 'Style', 'Model', 'Number of Items', 'Size', 'Material Type', 'Item Package Quantity', '重量', '尺寸', '上架时间', '全部流量词', '自然搜索词', '广告流量词', '搜索推荐词', '关键词反查', '国家', '标题', 'Collection time']
        ...
        if key in data_string:
            ...
            if next_key_index is not None:
                ...
                if key == "ASIN":
                    ...
                    data_dict.update(result)
                elif key in ("卖家数", "近30天销量(父体)", "近30天销量(子体)", "变体数", "全部流量词", "自然搜索词", "广告流量词", "搜索推荐词"): 
                    data_dict[key] = int(value.strip().replace(',','').replace('+','').replace('N/A','0').replace('<','').replace(' ','').replace('-', '0'))
                    if key in ("近30天销量(父体)", "近30天销量(子体)"):
                        sell_num += data_dict[key]
                        data_dict["销售数量"] = sell_num
                elif key == "关键词反查":
                    pass
                elif key == "Collection time":
                    current_datetime = datetime.now()
                    data_dict["Collection time"] = current_datetime.strftime("%Y-%m-%d %H:%M:%S")
                else:
                    data_dict[key] = value.strip()
    ...
    if "销售数量" in data_dict:
        data_dict["销售数量"] = data_dict["近30天销量(父体)"] if data_dict["近30天销量(父体)"] > data_dict["近30天销量(子体)"] else data_dict["近30天销量(子体)"]
    return data_dict
