    ...
    try:
        with connection.cursor() as cursor:
            for index, row in df.iterrows():
                ...
                if result:
                    # 如果存在记录，则更新
                    sql_update = "UPDATE goods SET URL = %s, 主图 = %s, 标题 = %s, 配送 = %s, 品牌 = %s, 卖家 = %s, 国家 = %s, 卖家数 = %s, 上架日期 = %s, 上架天数 = %s, 是否新品 = %s, 大类 = %s, 大类BSR = %s, 子类 = %s, 子类BSR = %s, 评分 = %s, 评分数 = %s, 评分段 = %s, 变体数 = %s, 销售数量 = %s, 价格 = %s, 销售额 = %s, '近30天销量父体' = %s, '近30天销量子体' = %s, 毛利率 = %s, FBA费用 = %s, 全部流量词 = %s, 自然搜索词 = %s, 广告流量词 = %s, 搜索推荐词 = %s, 重量 = %s, 尺寸 = %s, Color = %s, Style = %s, Coupon = %s, Size = %s, `Pattern Name` = %s WHERE ASIN = %s AND `Collection time` = %s"
                    ...
                    # 如果不存在记录，则插入
                    sql_insert = "INSERT INTO goods (ASIN, URL, 主图, 标题, 配送, 品牌, 卖家, 国家, 卖家数, 上架日期, 上架天数, 是否新品, 大类, 大类BSR, 子类, 子类BSR, 评分, 评分数, 评分段, 变体数, 销售数量, 价格, 销售额, 近30天销量父体, 近30天销量子体, 毛利率, FBA费用, 全部流量词, 自然搜索词, 广告流量词, 搜索推荐词, 重量, 尺寸, Color, Style, `Collection time`, Coupon, Size, `Pattern Name`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                    # sql_insert = "INSERT INTO goods (ASIN, URL, 主图, 标题, `Collection time`) VALUES (%s, %s, %s, %s, %s)"
                    # cursor.execute(sql_insert, (asin, row['URL'], row['主图'], row['标题'], collection_time))
                    cursor.execute(sql_insert, (asin, row['URL'], row['主图'], row['标题'], row['配送'], row['品牌'], row['卖家'], row['国家'], row['卖家数'], row['上架日期'], row['上架天数'], row['是否新品'], row['大类'], row['大类BSR'], row['子类'], row['子类BSR'], row['评分'], row['评分数'], row['评分段'], row['变体数'], row['销售数量'], row['价格'], row['销售额'], row['近30天销量父体'], row['近30天销量子体'], row['毛利率'], row['FBA费用'], row['全部流量词'], row['自然搜索词'], row['广告流量词'], row['搜索推荐词'], row['重量'], row['尺寸'], row['Color'], row['Style'], collection_time, row['Coupon'], row['Size'], row['Pattern Name']))
        connection.commit()
